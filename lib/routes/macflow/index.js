...
module.exports = async (ctx) => {
    ...
    const resultItem = await Promise.all(
        count.map(async (i) => {
            ...
            // var pubDate;
            // if (date.endsWith("天前")) {
            //     pubDate = moment().subtract(date.slice(0, 1), "days").hours(0).minutes(0).seconds(0).milliseconds(0);
            // } else {
            //     pubDate = moment(date, 'YYYY年MM月DD日').hours(0).minutes(0).seconds(0).milliseconds(0);
            // }
            const item = {
                title: a.text(),
                link: a.attr("href"),
                pubDate: $(list[i]).find(".date").text()
            };
